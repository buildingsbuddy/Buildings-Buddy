...
import { LayoutDashboard, Calculator, FolderOpen, CreditCard, ChevronLeft, ChevronRight, Building2 } from 'lucide-react';
    ...
    <aside className={cn(
      ...
      {!collapsed && sub.status === 'trial' && (
        <div className="mx-3 mb-3 p-3 rounded-lg bg-sidebar-primary/10 border border-sidebar-primary/20">
          <p className="text-xs font-semibold text-sidebar-primary">{sub.trialDaysLeft} days left on trial</p>
          <Link to="/billing" className="text-xs text-sidebar-foreground/60 hover:text-sidebar-primary mt-1 block">
            Upgrade now →
          </Link>
        </div>
      )}
