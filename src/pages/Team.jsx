...
const hasRealTeam = isValidUuid(team?.id);
...
if (!hasRealTeam) {
toast.error('Team invites need a real company team. Dev test bypass cannot send invites.');
return;
}
...
if (!hasRealTeam) {
toast.error('Dev test team cannot manage invites.');
return;
}
...
if (!hasRealTeam) {
toast.error('Dev test team cannot manage members.');
return;
}
...
Developer test team active
</p>
<p className="text-sm text-yellow-800 mt-1">
Your subscription bypass is active, but this is not a real Supabase team.
Calculators, pricing, saving and PDF export can be tested. Team invites need
a real company team record.
</p>
</div>
</CardContent>
</Card>
...
disabled={seatsRemaining <= 0 || !hasRealTeam}
/>
...
Team invites are disabled while using the developer test bypass.
</p>
) : seatsRemaining <= 0 ? (
...
disabled={removingId === member.id || !hasRealTeam}
...
disabled={removingId === invite.id || !hasRealTeam}
