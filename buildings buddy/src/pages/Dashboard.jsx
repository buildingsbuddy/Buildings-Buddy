            ...
            <CardContent className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-accent/10 flex items-center justify-center">
                  {sub.status === 'trial' ? <Clock className="w-5 h-5 text-accent" /> : <AlertCircle className="w-5 h-5 text-accent" />}
                </div>
                <div>
                  {sub.status === 'trial' ? (
                    <>
                      <p className="font-semibold">{sub.trialDaysLeft} days remaining on your free trial</p>
                      <p className="text-sm text-muted-foreground">Upgrade to keep uninterrupted access to all calculators.</p>
                    </>
                  ) : (
                    <>
                      <p className="font-semibold">{sub.status === 'no_subscription' ? 'Start your free trial' : 'Your access has expired'}</p>
                      <p className="text-sm text-muted-foreground">Subscribe to unlock all material calculations.</p>
                    </>
                  )}
                </div>
              </div>
              ...
              <Card key={p.id} className="hover:bg-muted/30 transition-colors">
                <CardContent className="p-4 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground capitalize">{p.calculator_type?.replace(/_/g, ' ')}</p>
                  </div>
